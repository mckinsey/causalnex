...
class TestFitNodeStates:
    ...
    def test_all_nodes_included(self, weighted_edges, data):
        ...
        bn = DynamicBayesianNetwork(cg).fit_node_states(data)
    ...
    def test_all_states_included(self):
        ...
        bn = DynamicBayesianNetwork(cg).fit_node_states(
    ...
    def test_fit_with_null_states_raises_error(self):
        ...
        with pytest.raises(ValueError, match="node '.*' contains None state"):
            DynamicBayesianNetwork(cg).fit_node_states(
...
class TestFitNodesStatesAndCPDs:
    ...
    def test_behaves_same_as_separate_calls(self, train_data_idx, train_data_discrete):
        bn1 = DynamicBayesianNetwork(from_pandas(train_data_idx, w_threshold=0.3))
        bn2 = DynamicBayesianNetwork(from_pandas(train_data_idx, w_threshold=0.3))
...
class TestLatentVariable:
    ...
    def test_em_algorithm(self):  # pylint: disable=too-many-locals
        """
        Test if `DynamicBayesianNetwork` works with EM algorithm.
        ...
        complete_bn = DynamicBayesianNetwork(
        ...
        bn = DynamicBayesianNetwork(sm_no_lv)
        bn.fit_node_states(data)
        bn.fit_cpds(data)
...
class TestInit:
    """Test behaviour when constructing a DynamicBayesianNetwork"""
    ...
    def test_cycles_in_structure(self):
        ...
        with pytest.raises(
            ...
            DynamicBayesianNetwork(StructureModel([(0, 1), (1, 2), (2, 0)]))
    ...
    def test_disconnected_components(self, test_input, n_components):
        ...
        with pytest.raises(
            ...
            DynamicBayesianNetwork(StructureModel(test_input))
