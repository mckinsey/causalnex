...
def train_data_bn_2():
    """
    Data for training a bayesian network.
    Data was generated randomly with the following configuration:
    - data points: 50,
    - variables: 5,
    ...
    We provide a dictionary with:
    - adjacency matrix - `W`
    - data as an array (for from_numpy) `X_array`
    - data as an pd.DataFrame (for from_pandas) `X_pandas`
    """
    data = {
        "W": np.array(
            [
    ...
    data["X_pandas"] = pd.DataFrame(data["X_array"], columns=["a", "b", "c", "d", "e"])
    return data
